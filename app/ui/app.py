"""
Main UI application window.

Provides the RefereeApp Tkinter GUI with pages for game tracking, rulebook,
testing, and statistics. Supports both database and JSON-based data storage.
"""
import sys
import os
import random
...
from app.rulebook.content import load_handball_dataset
...
class RefereeApp(tk.Frame):
    """Main application window for the Referee Earnings Tracker.

    A comprehensive Tkinter GUI providing:
    - Game tracking: Add, edit, delete, and search game records
    - Earnings management: Track payments, transportation, and food costs
    - Rulebook: Browse and search handball rules with embedded images
    - Testing: Take quizzes to improve referee knowledge
    - Statistics: View performance and earnings summaries by season

    Supports multiple data backends: SQLite, MySQL, PostgreSQL, or JSON files.

    Attributes:
        root: Tkinter root window.
        manager: GameDataManager instance for data persistence.
        seasons: List of available seasons.
        selected_season: Currently selected season (StringVar).
        games: Cached list of games for current season.
        rulebook_dataset_path: Path to loaded rulebook dataset.
    """
    ...
    def _setup_theme(self):
        # Modern Material Design palette with clean lines and better contrast
        self.colors = {
            "root_bg": "#121212",  # Deep dark background
            "sidebar_bg": "#1f1f1f",  # Slightly lighter sidebar
            "sidebar_text": "#ffffff",
            "sidebar_muted": "#b0b0b0",
            "sidebar_active": "#2196F3",  # Material Blue
            "shell_bg": "#f5f5f5",  # Light gray background
            "surface": "#ffffff",  # White cards
            "surface_alt": "#f9f9f9",  # Very light gray
            "text_primary": "#212121",  # Nearly black
            "text_secondary": "#757575",  # Medium gray
            "hero_bg": "#2196F3",  # Material Blue
            "hero_sub": "#E3F2FD",  # Light blue
            "rulebook_bg": "#f5f5f5",
            "rulebook_panel": "#1976D2",  # Darker blue
            "ball_main": "#42A5F5",  # Light blue
            "ball_soft": "#BBDEFB",  # Very light blue
            "accent": "#FF6F00",  # Material Amber
            "accent_soft": "#FFE0B2",  # Light amber
    ...
    def setup_ui(self):
        ...
        self.last_test_session = None
        self.test_timer_job = None
        self.test_end_epoch = None
        self.test_total_seconds = 4 * 60
    ...
    def _build_tests_page(self):
        ...
        tk.Label(rec_body, text="Randomized test (4 questions)", bg=c["surface_alt"], fg=c["text_primary"], font=f["h3"]).pack(side="left", padx=16)
        tk.Button(
            rec_body,
            text=">",
            bg=c["success"],
            ...
            font=f["h2"],
            width=3,
            command=lambda: self.start_test_session(mode="random", total=4),
        ...
        # Review button
        self.review_button_frame = tk.Frame(self.tests_launcher, bg=c["shell_bg"])
        self.review_button_frame.pack(fill="x", pady=(12, 0))
        ttk.Button(self.review_button_frame, text="Review Last Test", command=self.show_test_review).pack(side="left")
        self.review_status_var = tk.StringVar(value="")
        ttk.Label(self.review_button_frame, textvariable=self.review_status_var, foreground="gray").pack(side="left", padx=12)

        # Review panel (hidden by default)
        self.test_review_panel = tk.Frame(page, bg=c["surface"], bd=1, relief="solid", highlightbackground=c["border"], highlightthickness=1)
        self.test_review_panel.pack(fill="both", expand=True, padx=16, pady=(4, 14))
        self.test_review_panel.pack_forget()
    ...
    def _render_primary_test_modes(self):
        ...
        modes = [
            ("RAND", "Randomized test (30 questions)", lambda: self.start_test_session(mode="random", total=4)),
            ("NEW", "Unanswered questions", lambda: self.start_test_session(mode="unanswered", total=4)),
            ("WRONG", "Questions answered wrong", lambda: self.start_test_session(mode="wrong", total=4)),
        ]
    ...
    def _render_test_categories(self):
        ...
        for rule_id in shown:
            ...
            tk.Button(
                ...
                font=f["h3"],
                width=3,
                command=lambda r=rule_id: self.start_test_session(mode="rule", rule=r, total=4),
            ).pack(side="right", fill="y")
    ...
    def start_test_session(self, mode="random", rule="", total=4):
        if not self.quiz_questions:
    ...
    def finish_test_session(self, timeout=False):
        ...
        # Store the actual questions and results for review
        questions_with_results = []
        for q in self.active_test_questions:
            qid = str(q.get('id', ''))
            result = self.session_results.get(qid, 'unanswered')
            questions_with_results.append({
                'id': qid,
                'question': q.get('question', ''),
                'rule_references': q.get('rule_references', []),
        ...
        # Store last session for review
        self.last_test_session = session_entry
    ...
    def show_test_review(self):
        """Display the last test session with rule content for each question."""
        if not hasattr(self, 'last_test_session') or not self.last_test_session:
            messagebox.showinfo("Review", "No test session to review yet.")
            return

        session = self.last_test_session
        questions = session.get('questions_with_results', [])

        if not questions:
            messagebox.showinfo("Review", "No questions to review.")
            return

        # Clear the review panel
        for w in self.test_review_panel.winfo_children():
            w.destroy()
        ...
        # Header
        header = ttk.Frame(self.test_review_panel)
        header.pack(fill="x", padx=12, pady=(10, 6))
        ttk.Button(header, text="Back", command=self.hide_test_review).pack(side="left")
        verdict = "PASSED" if session.get('passed') else "FAILED"
        title = f"Test Review: {verdict} ({session.get('correct')}/{session.get('total')} correct)"
        ttk.Label(header, text=title, font=f["h2"]).pack(side="left", padx=12)

        # Scrollable content
        canvas_frame = tk.Frame(self.test_review_panel, bg=c["shell_bg"])
        canvas_frame.pack(fill="both", expand=True, padx=12, pady=12)

        canvas = tk.Canvas(canvas_frame, bg=c["shell_bg"], highlightthickness=0)
        ...
        inner_frame = tk.Frame(canvas, bg=c["shell_bg"])
        canvas_window = canvas.create_window(0, 0, window=inner_frame, anchor="nw")
        inner_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))

        # Display each question with result and related rules
        for idx, q_data in enumerate(questions, 1):
            result = q_data.get('result', 'unanswered')
            q_title = q_data.get('question', '')

            # Question card
            card = tk.Frame(inner_frame, bg=c["surface_alt"], bd=1, relief="solid", highlightbackground=c["border"], highlightthickness=1)
            card.pack(fill="x", pady=(0, 8))

            # Result indicator
            result_bg = c["success"] if result == "correct" else (c["error"] if result == "wrong" else c["warning"])
            result_text = "✓" if result == "correct" else ("✗" if result == "wrong" else "?")
            tk.Label(card, text=f"Q{idx}", bg=result_bg, fg=c["surface"], font=f["h3"], width=3).pack(side="left", fill="y")

            # Question text
            q_frame = tk.Frame(card, bg=c["surface_alt"])
            q_frame.pack(side="left", fill="both", expand=True, padx=12, pady=8)
            tk.Label(q_frame, text=q_title, bg=c["surface_alt"], fg=c["text_primary"], wraplength=800, justify="left", font=f["body"]).pack(anchor="w")

            # Rule references with content
            rule_refs = q_data.get('rule_references', [])
            if rule_refs:
                tk.Label(q_frame, text="Related Rules:", bg=c["surface_alt"], fg=c["text_secondary"], font=f["small"]).pack(anchor="w", pady=(6, 3))

                for rule_ref in rule_refs:
                    rule_section = self._find_rule_section(rule_ref)
                    if rule_section:
                        rule_frame = tk.Frame(q_frame, bg=c["shell_bg"], bd=1, relief="flat")
                        rule_frame.pack(anchor="w", fill="x", padx=8, pady=2)

                        # Rule title
                        tk.Label(rule_frame, text=f"{rule_ref}: {rule_section.get('title', '')}", bg=c["shell_bg"], fg=c["accent"], font=f["small_bold"]).pack(anchor="w", padx=4, pady=(4, 2))

                        # Rule content
                        content = rule_section.get('content', '')
                        if content:
                            display_content = content[:400] + "..." if len(content) > 400 else content
                            tk.Label(rule_frame, text=display_content, bg=c["shell_bg"], fg=c["text_secondary"], wraplength=700, justify="left", font=f["small"]).pack(anchor="w", padx=4, pady=(0, 4))

        # Update review status
        self.review_status_var.set(f"Showing {len(questions)} questions")

        # Show review panel, hide launcher
        self.tests_launcher.pack_forget()
        self.test_review_panel.pack(fill="both", expand=True, padx=16, pady=(4, 14))

    def hide_test_review(self):
        """Hide the review panel and show the launcher again."""
        self.test_review_panel.pack_forget()
        self.tests_launcher.pack(fill="x", padx=16, pady=(14, 8))
    ...
    def _build_rulebook_page(self):
        ...
        self.rulebook_query_var = tk.StringVar()
        entry = ttk.Entry(search_row, textvariable=self.rulebook_query_var)
        entry.pack(side="left", fill="x", expand=True, padx=6)
        entry.bind('<Return>', lambda e: self.search_rulebook_ui())
        ttk.Button(search_row, text="Search", command=self.search_rulebook_ui).pack(side="left")
        ttk.Button(search_row, text="Clear", command=self._clear_rulebook_search).pack(side="left", padx=2)

        split = ttk.Frame(page)
        split.pack(fill="both", expand=True, padx=12, pady=(6, 12))

        # Left panel: Rule index (always visible)
        left_panel = tk.Frame(split, bg=c["shell_bg"])
        left_panel.pack(side="left", fill="y", padx=(0, 8))

        # Index header
        tk.Label(left_panel, text="Table of Contents", bg=c["shell_bg"], fg=c["text_primary"], font=f["body_bold"]).pack(fill="x", padx=4, pady=(0, 6))

        # List container frame with scrollbar
        list_container = tk.Frame(left_panel, bg=c["shell_bg"])
        list_container.pack(side="left", fill="both", expand=True)

        # Index list (all rules in order)
        self.rulebook_index_list = tk.Listbox(
            list_container,
            width=32,
            bg=c["surface"],
            fg=c["text_primary"],
        ...
        self.rulebook_index_list.bind("<<ListboxSelect>>", lambda e: self._select_from_index())
        self.rulebook_index_list.bind("<MouseWheel>", self._on_rulebook_list_mousewheel)

        # Search results list (hidden by default, shown only during search)
        ...
        # Scrollbar (works with whichever list is visible)
        list_scroll = ttk.Scrollbar(list_container, orient="vertical")
        list_scroll.pack(side="right", fill="y")
        self.rulebook_index_list.configure(yscrollcommand=list_scroll.set)
        self.rulebook_results_list.configure(yscrollcommand=list_scroll.set)
        ...
        ttk.Checkbutton(
            ...
            command=self._render_rulebook_content,
    ...
    def _find_rule_section(self, rule_ref: str):
        """Find a rule section in the handball dataset by reference.

        Question references look like '1:1', '13:4a', etc., where the part
        before the colon is the rule (section_id) number. Rulebook sections
        use section_id like '1', '13'. Match on the rule number prefix.
        """
        if not self.handball_dataset or not self.handball_dataset.get('rulebook'):
            return None

        # Take the rule number before any ':' (e.g. '13:4a' -> '13')
        rule_num = str(rule_ref).split(':', 1)[0].strip().lower()

        rulebook = self.handball_dataset['rulebook']
        for section in rulebook:
            sid = str(section.get('section_id', '')).strip().lower()
            if sid == rule_num:
                return section
    ...
    def _update_rule_info_status(self, section: Dict[str, Any]):
        """Update status bar with rule metadata (difficulty, question count, keywords)."""
    ...
    def _select_from_index(self):
        """Handle selection from rule index - jump to selected rule in PDF."""
        if not self.handball_dataset or not self.handball_dataset.get("rulebook"):
            return

        sel = self.rulebook_index_list.curselection()
        if not sel:
            return

        # Get selected rule from dataset (matches index order)
        section = self.handball_dataset["rulebook"][sel[0]]
        ...
        # Jump to rule's start page and update info bar
        ...
        self._render_rulebook_content()
        self._update_rule_info_status(section)

    def _render_rulebook_content(self):
        """Render rulebook PDF page."""
        self._render_rulebook_pdf_page()

    def _render_rulebook_text_view(self):
        """Render extracted text and images from dataset."""
        if not self.rulebook_current_section:
            # Clear text view
            for widget in self.rulebook_text_inner.winfo_children():
                widget.destroy()
            return
        ...
        if continuous and self.rulebook_current_section:
            # Continuous mode: show all related sections
            sections_to_show = [self.rulebook_current_section]
        else:
            # Single section mode
            sections_to_show = [self.rulebook_current_section]
        ...
        for section in sections_to_show:
            ...
            # Separator
            tk.Frame(self.rulebook_text_inner, bg=c["border"], height=1).pack(fill="x", padx=12, pady=6)
            ...
            if content:
                ...
                text_widget.pack(fill="both", padx=12, pady=6)
            ...
            if images:
                ...
                for img_path in images:
                    try:
                        full_path = Path(__file__).resolve().parent.parent / img_path
                        if full_path.exists():
                            ...
                            photo = ImageTk.PhotoImage(img)
                            img_label = tk.Label(self.rulebook_text_inner, image=photo, bg=c["shell_bg"])
                            img_label.image = photo  # Keep a reference
                            img_label.pack(fill="both", padx=12, pady=6)
                    except Exception:
                        # Image load failed, show path instead
                        tk.Label(
                            self.rulebook_text_inner,
            ...
            if continuous:
                tk.Frame(self.rulebook_text_inner, bg=c["shell_bg"], height=20).pack(fill="x")

        # Update scroll region
        self.rulebook_text_inner.update_idletasks()
        self.rulebook_text_canvas.configure(scrollregion=self.rulebook_text_canvas.bbox("all"))

    def _load_rulebook_index(self):
        """Populate the rule index with all rules in order."""
        if not self.handball_dataset or not self.handball_dataset.get("rulebook"):
            return

        self.rulebook_index_list.delete(0, tk.END)
        for idx, section in enumerate(self.handball_dataset["rulebook"]):
            sid = section.get("section_id", "")
            title = section.get("title", "")

            # Format: "Rule X - Title"
            display_text = f"{sid} - {title}"
            self.rulebook_index_list.insert(idx, display_text)

    def _clear_rulebook_search(self):
        """Clear search and return to full rule index."""
        self.rulebook_query_var.set("")
        self.rulebook_results_list.delete(0, tk.END)
        self.rulebook_results = []

        # Hide search results, show index
    ...
    def submit_test_answer(self):
        ...
        # During test, show no feedback - only reveal results after test finishes
        self.test_feedback_var.set("")
    ...
    def load_question_of_day(self):
        ...
        # Check if we already have a QOD for today
        today = date.today().isoformat()
        qod_cache = self.test_progress.get('qod_cache', {})

        if qod_cache.get('date') == today and qod_cache.get('question_id'):
            # Use cached question from today
            qid = qod_cache['question_id']
            self.qod_question = next((q for q in self.quiz_questions if str(q.get('id')) == str(qid)), None)
            if not self.qod_question:
                # Cached question not found, pick a new one
                self.qod_question = random.choice(self.quiz_questions)
        else:
            # New day, pick a random question
            self.qod_question = random.choice(self.quiz_questions)
            # Cache it
            self.test_progress['qod_cache'] = {
                'date': today,
                'question_id': str(self.qod_question.get('id', ''))
            }
            self._save_test_progress()
        q = self.qod_question
    ...
    def submit_question_of_day(self):
        ...
        feedback_lines = [
            f"{verdict}. Your answer: {', '.join(selected) or '-'} | Expected: {', '.join(correct) or '-'}"
        ]
    ...
    def search_rulebook_ui(self):
        ...
        query = self.rulebook_query_var.get().strip()
        if not query:
            if hasattr(self, 'rulebook_pdf_status_var'):
                self.rulebook_pdf_status_var.set("Type a keyword to search the rulebook.")
            return

        self.rulebook_results = search_rulebook(self.handball_dataset, query)
        if not self.rulebook_results:
            if hasattr(self, 'rulebook_pdf_status_var'):
                self.rulebook_pdf_status_var.set(f"No matches for '{query}'.")
            return

        # Hide index, show search results
        if hasattr(self, 'rulebook_index_list'):
            self.rulebook_index_list.pack_forget()
        if hasattr(self, 'rulebook_results_list'):
            self.rulebook_results_list.pack(side="left", fill="both", expand=True)

        # Display results with difficulty and question count
        for idx, section in enumerate(self.rulebook_results):
            ...
            # Format: "Rule X - Title [Difficulty] (N q)"
            display_text = f"{sid} - {title}"
            if difficulty and difficulty != "Untested":
    ...
    def show_selected_rule_section(self):
        ...
        # Update status bar with relevant rule metadata
        self._update_rule_info_status(section)
