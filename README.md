...
### Handball quiz/rulebook JSON pipeline
- You can convert 3 PDF sources into a single JSON file for a future quiz mode:
  - PDF 1: Questions
  - PDF 2: Answers with reasoning
  - PDF 3: Rulebook
- Install dependencies (includes pypdf):
  - pip install -r requirements.txt
- Build the JSON dataset:
  - python scripts/build_handball_json.py --questions "path/to/questions.pdf" --answers "path/to/answers.pdf" --rulebook "path/to/rulebook.pdf" --output "data/handball_content.json"
- Output format:
  - metadata (source files and counts)
  - questions (id, question text, options, answer, reasoning)
  - rulebook sections (section id, title, content)

This JSON can be loaded by the app to implement:
- test mode (question by question)
- answer checking with reasoning display
- rulebook search by keyword
